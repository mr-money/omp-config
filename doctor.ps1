...
$bundle = Join-Path $homeDir ".bun\install\global\node_modules\@oh-my-pi\pi-coding-agent\dist\cli.js"
...
$src=if(Test-Path $bundle){Get-Content $bundle -Raw}else{""}; Check "CNY patch" ($src -match '__CNY_PATCH_VERSION__=.*2026\.08\.23\.1' -and $src -match '__cnyIsFree') "patch marker"
$wrapper=Join-Path $homeDir ".bun\bin\omp.cmd"; Check "wrapper" (Test-Path $wrapper) $wrapper
