...
function ensureCompatibleBundleVersion() {
	...
	try {
		...
		if (cmpVersion(parseVersion(again.version), MIN_BUNDLE_VER) < 0) {
			fail(`omp upgrade did not reach minimum ${minStr} (actual ${again.version || "unknown"})`);
		}
		log(`omp bundle now ${again.version}`);
		return true;
	} catch (e) {
		fail(`omp version check/upgrade failed: ${e.code || e.message}`);
	}
}
