...
def _recalc_with_profile(filename, abs_path, timeout, profile_dir: Path):
    ...
    if platform.system() == "Windows":
        # The vnd.sun.star.script macro invocation hangs on Windows. Recalculate
        # via convert-to xlsx instead, which evaluates formulas and stores cached
        # values; the produced file is copied back over the original below.
        out_dir = profile_dir / "converted"
        out_dir.mkdir()
        cmd = [
            "soffice",
            "--headless",
    ...
    try:
        if platform.system() == "Windows":
            # soffice.exe is a GUI launcher on Windows; its soffice.bin child
            # can hold the pipe handles open, defeating the timeout. Redirect
            # to files instead of pipes.
            with tempfile.TemporaryFile() as out_f, tempfile.TemporaryFile() as err_f:
                ...
                out_f.seek(0)
                err_f.seek(0)
                result.stdout = out_f.read().decode(errors="replace")
                result.stderr = err_f.read().decode(errors="replace")
        else:
            result = subprocess.run(
    ...
    if platform.system() == "Windows":
        ...
        if not produced.exists():
            return {"error": "LibreOffice exited cleanly but produced no recalculated file"}
        try:
            shutil.copy2(produced, abs_path)
        except OSError as e:
            return {"error": f"Could not copy the recalculated workbook back over the original: {e}"}
