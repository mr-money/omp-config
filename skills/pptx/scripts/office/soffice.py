...
def run_soffice(args: Iterable[str], **kwargs) -> subprocess.CompletedProcess:
    ...
    with contextlib.ExitStack() as stack:
        ...
        if platform.system() == "Windows" and kwargs.pop("capture_output", False):
            # On Windows, soffice.exe is a GUI-subsystem launcher that spawns
            # soffice.bin; the child can keep the stdout/stderr pipe handles
            # open, so communicate() blocks past any timeout. Redirect to files.
